...
async function testslibsodium() {
  ...
  let verkey; let sigkey;

  /*
  Generating Keypair without seed
  */
  const keypair = await libsodium.crypto_sign_keypair();
  ...
  */
  sigseed = libsodium.crypto_pwhash(32,
    '',
    salt,
  ...
  const seedKeypair = libsodium.crypto_sign_seed_keypair(sigseed);
  ...
  */
  ...
  const acypher = await libsodium.crypto_box_easy(atob_txb, nonce, bob_pubkey, alice_prikey);
