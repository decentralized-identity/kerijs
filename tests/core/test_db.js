...
const fs = require('fs');
const { snkey, dgkey, Databaser } = require('../../src/keri/db/database');
const { versify, Serials } = require('../../src/keri/core/core');
const { openDatabaser, openLogger, Logger } = require('../../src/keri/db/logger');
...
function test_opendatabaser() {
  ...
  const dberGen = openDatabaser('test', null);
  ...
  const vals = [Buffer.from('z', 'binary'), Buffer.from('m', 'binary'), Buffer.from('x', 'binary'), Buffer.from('a', 'binary')];
  //
